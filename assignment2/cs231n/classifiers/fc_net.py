...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for layer in range(self.num_layers):
            ...
            self.params['W'+str(layer+1)] = np.random.randn(input_dim, hidden_dim)* weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(self.num_layers-1):
            ...
            if self.use_batchnorm:
                ...
                a, caches[layer+1] = affine_bn_relu_forward(input_X, W, b, gamma, beta, self.bn_params[layer])
            
            else:        
                a, caches[layer+1] = affine_relu_forward(input_X, W, b)
            
            if self.use_dropout:
                a, cache_dropout = dropout_forward(a, self.dropout_param)
                caches[layer+1]  =  caches[layer+1] + (cache_dropout,)
            
            input_X = a
        
        # -affine-softmax
        W, b = self.params['W'+str(self.num_layers)], self.params['b'+str(self.num_layers)]            
        scores, caches[self.num_layers] = affine_forward(input_X, W, b)
        ...
        for layer in np.arange(self.num_layers, 0, -1):
            if layer == self.num_layers:
                dx, dw, db = affine_backward(dout, caches[layer])          
            else:
                if self.use_dropout:
                    dropout_cache = caches[layer][-1]
                    dout = dropout_backward(dout, dropout_cache)
                    
                    caches[layer] = tuple(list(caches[layer])[:-1])
                    
                if self.use_batchnorm:
                    dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dout, caches[layer])
                    ...
                    dx, dw, db = affine_relu_backward(dout, caches[layer])
